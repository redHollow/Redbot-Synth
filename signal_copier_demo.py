...
def check_symbol_blocked(symbol):
    ...
    if symbol in daily_symbol_losses:
        entry = daily_symbol_losses[symbol]
        if entry['date'] == today and entry['losses'] > 0:
            return True
    return False
...
def record_symbol_loss(symbol):
    ...
    daily_symbol_losses[symbol]['losses'] += 1
                    ...
                    if deals:
                        for deal in deals:
                            if deal.magic >= MAGIC_BASE and deal.entry == 1:  # DEAL_ENTRY_OUT
                                ...
                                if profit < 0:
                                    record_symbol_loss(sym)
                                msg = f"VIP {result}: ${profit:+.2f} on {sym}"
                                if profit < 0 and SKIP_AFTER_LOSS:
                                    msg += f" (⛔ {sym} blocked for today)"
                                log.info(msg)
